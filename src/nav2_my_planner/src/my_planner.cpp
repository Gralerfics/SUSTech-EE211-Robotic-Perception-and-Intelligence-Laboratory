...
namespace nav2_my_planner {
    ...
    AStar::AStar() : tf_(nullptr), costmap_(nullptr) {
        RCLCPP_INFO(logger_,
                    "AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar, AStar ~");
    }
    ...
    nav_msgs::msg::Path AStar::createPlan(
        ...
        if (!costmap_->worldToMap(start.pose.position.x, start.pose.position.y, mx_start, my_start)) {
            RCLCPP_ERROR(logger_, "The start position (%f, %f) is off the map.", start.pose.position.x,
                         start.pose.position.y);
        }
        if (!costmap_->worldToMap(goal.pose.position.x, goal.pose.position.y, mx_goal, my_goal)) {
            RCLCPP_ERROR(logger_, "The goal position (%f, %f) is off the map.", goal.pose.position.x,
                         goal.pose.position.y);
        }
        if (tolerance_ == 0 && costmap_->getCost(mx_goal, my_goal) == nav2_costmap_2d::LETHAL_OBSTACLE) {
            RCLCPP_ERROR(logger_, "The goal (%f, %f) was in a lethal cost.", goal.pose.position.x,
                         goal.pose.position.y);
        }
        ...
        if (!makePlan(start.pose, goal.pose, tolerance_, path))
            RCLCPP_ERROR(logger_, "Failed to create plan with tolerance of: %f", tolerance_);

        return path;
    }

    bool AStar::isPlannerOutOfDate() {
        return (!planner_.get() || planner_->nx != static_cast<int>(costmap_->getSizeInCellsX()) ||
    ...
    bool AStar::worldToMap(double wx, double wy, unsigned int &mx, unsigned int &my) {
        ...
        RCLCPP_ERROR(logger_, "worldToMap failed: mx,my: %d,%d, size_x,size_y: %d,%d", mx, my,
