...
def main():
    ...
    goal_SA = PoseStamped()
    goal_SA.header.frame_id = 'map'
    goal_SA.header.stamp = navigator.get_clock().now().to_msg()
    goal_SA.pose.position.x = -0.14
    goal_SA.pose.position.y = -1.807
    goal_SA.pose.orientation.z = -0.6686671777907486
    goal_SA.pose.orientation.w = 0.7435618369344646
    ...
    goal_AB = PoseStamped()
    goal_AB.header.frame_id = 'map'
    goal_AB.header.stamp = navigator.get_clock().now().to_msg()
    goal_AB.pose.position.x = 1.55
    goal_AB.pose.position.y = -3.72
    goal_AB.pose.orientation.z = -0.007822402212393793
    goal_AB.pose.orientation.w = 0.9999694045437728
    ...
    # Go to A
    # navigator.go_to_goal(goal_A, blocking = False) # raw
    navigator.go_to_goal(goal_SA)
    ...
    navigator.go_to_goal(goal_AB)
    navigator.go_to_goal(goal_B) # raw
